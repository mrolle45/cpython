...
Utility functions involving unicode codepoints.  Conforms to Unicode standard
version 16.0.

This is a separate module so that it can be imported only when an actual
unicode codepoint is seen and it needs to be analyzed.
'''

from __future__ import annotations

from pcpp.common import *
from pcpp.escape import escapes

__all__ = "Uni".split()

@functools.cache
def Uni(lang: Preprocessor.Language) -> UniBase:
    """
    Factory for singleton object which performs various functions.  The
    instance is a subclass of UniBase, which depends on preprocessor
    attributes.
    """
    inst: UniBase
    if lang.cplus_ver or lang.c_ver >= 2023:
        if lang.clang:
            inst = UniCppClang(lang)
        else:
            inst = UniCpp(lang)
    else:
        if lang.clang:
...
def ident(lang: Preprocessor.Language, unistart: str, cont_group: str,
          t: LexToken, m: re.Match,
          ) -> None:
    """
    Handle a lex.lex token which contains a unicode character or escape.  The
    type and value of the token may be modified.
    """
    uni: UniBase = Uni(lang)
    if unistart:
        codepoint: int = uni.get_codepoint(
            unistart, uni.char_uni_start.match(unistart), 'numstart')
        if codepoint is not None and uni.ident_start(codepoint):
            if cont_group:
                uni.ident_cont(t, m)
        else:
            t.value = unistart
            ...
            if True:
            #if unistart[0] != '\\':
                lang.prep.on_error_token(t,
                                         f"Illegal character {t.value!r}")
            # Report the error.
    elif cont_group:
...
class UniBase:
    char_uni_cont: re.Pattern
    char_uni_start: re.Pattern

    def __init__(self, lang: Preprocessor.Language):
        self.lang = lang
        self.char_uni_cont = re.compile(lang.prep.lexer.REs.char_uni_cont)
        self.char_uni_start = re.compile(lang.prep.lexer.REs.char_uni_start)
        self.escapes = escapes(lang)

    def ident_cont(self, t: LexToken, m: re.Match) -> None:
        """
        Adjusts token value for continue codepoints.  An invalid codepoint
        will either truncate the value or just remove that codepoint.  Don't
        check the start codepoint, which is at the beginning of the value.
        """
        result = m.group()

        # First unicode match that truncates the final result.
        truncation: re.Match = None
        # How many chars skipped before the truncation.
        truncation_skips: int = 0

        # Callback for Pattern.sub() for each match of the pattern.
        def sub_cb(m: re.Match) -> str:
            """
            Replacement for matched unicode character or escape.  Also notes
            first truncation.  Ignore match at the beginning of the value.
            """
            nonlocal truncation, truncation_skips
            # Get the codepoint value.
            val: str = m.group()
            if m.start() == 0:
                return val
            try:
                codepoint = self.get_codepoint(val, m, 'numcont', skip=True)
                if codepoint is None or not self.ident_continue(codepoint):
                    # Codepoint not valid, truncating the result the first time.
                    if not truncation:
                        truncation = m
                return val
            except SkipException:
                # Codepoint not valid, but just ignoring it and moving on.
                if not truncation:
                    truncation_skips += len(val)
                return ''

        # Modify result for some unicode characters therein.
        result = self.char_uni_cont.sub(sub_cb, result)

        if truncation:
            # Truncate the result at this location.  This also truncates the
            # substring of lex data covered.
            pos: int = truncation.start()
            t.lexer.lexpos = t.lexpos + pos
            result = result[ : pos - truncation_skips]

        if truncation_skips:
            t.skip_len = truncation_skips
        t.value = result

    def ident_continue(self, codepoint: int | None) -> bool:
        """ True if codepoint is an identifier continue character. """
        # clang considers ALL codepoints as continue characters, so this
        # method is overridden in UniCClang and UniCppClang.
        return codepoint in self.continue_ranges

    def get_codepoint(self, val: str, m: re.Match, groupname: str,
                      skip: bool = False,
                      ) -> int | None:
        """ The numerical codepoint for the matched value. """
        if m.group(groupname):
            return ord(val[0])
        else:
            # The value is encoded in the escape sequence.
            e = self.escapes(val, m, uni=True)
            if skip and getattr(e.diag, 'skip', False):
                raise SkipException()
            if e.err:
                return None
            return e.value
...
class UniClang(UniBase):
    """ Methods which apply to clang. """
    def ident_continue(self, codepoint: int) -> bool:
        """
        True if codepoint is an identifier continue character, according to
        clang.  raise SkipException to cause the codepoint to be ignored and
        to keep on processing the token.
        """
        if 0xD800 <= codepoint < 0xDFFF:
            # Unicode surrogate character.
            return False
        if codepoint < 0xA0:
            # Unicode control character or nothing (-1).
            if codepoint < 0:
                raise SkipException()
            return False
        # Clang accepts everything else!  It doesn't check for XID_CONTINUE or
        # C99 Annex D.2.  However, a codepoint over the maximum is dropped
        # from the token.
...
class UniC(UniBase):
    non_start_ranges: Ranges
    continue_ranges: Ranges

    def __init__(self, lang: Preprocessor.Language):
        super().__init__(lang)
        # Using C standard Annex D for identifiers.  
        # These strings are copied directly from the C11 or C99 standard Annex
        # D at https://www.open-std.org/jtc1/sc22/wg14/www/docs/n1548.pdf.
        # Note, the dashes are actually \u2212.

        def decode_annex_D(data: str) -> Iterator[Range]:
            """
            Parse codepoint ranges from Annex D.1 or D.2  and return the
            ranges.
            """
            dash = '\u2212'
            hexpat = re.compile(
                rf'(?P<first>{hexre})({dash}(?P<last>{hexre}))?')
            for m in hexpat.finditer(data):
                first = m.group('first')
                last = m.group('last') or first
                yield Range(int(first, 16), int(last, 16) + 1)

        # C99 has a different Annex D from C11 et seq.
        ...
        if lang.c_ver < 2011:
            ...
            annex_D_allowed = """
                ...
                440 Universal character names for identifiers §D
                0B5C−0B5D, 0B5F−0B61
                ...
                0CAA−0CB3, 0CB5−0CB9, 0CBE−0CC4, 0CC6−0CC8, 0CCA−0CCD, 0CDE,
                0CE0−0CE1
                Malayalam: 0D02−0D03, 0D05−0D0C, 0D0E−0D10, 0D12−0D28,
                0D2A−0D39, 0D3E−0D43, 0D46−0D48, 0D4A−0D4D, 0D60−0D61
                ...
                0F49−0F69, 0F71−0F84, 0F86−0F8B, 0F90−0F95, 0F97, 0F99−0FAD,
                0FB1−0FB7, 0FB9
            ...
            """
            annex_D_disallowed_initially = """
                Digits: 0660−0669, 06F0−06F9, 0966−096F, 09E6−09EF, 0A66−0A6F,
                0AE6−0AEF, 0B66−0B6F, 0BE7−0BEF, 0C66−0C6F, 0CE6−0CEF,
                0D66−0D6F, 0E50−0E59, 0ED0−0ED9, 0F20−0F33
            """
            self.continue_ranges = Ranges(
                *sorted(decode_annex_D(annex_D_allowed)))
            self.non_start_ranges = Ranges(
                *sorted(decode_annex_D(annex_D_disallowed_initially)))
        else:
            # Annex D.1
            annex_D_allowed = """
            ...
            self.continue_ranges = Ranges(*decode_annex_D(annex_D_allowed))
            self.non_start_ranges = Ranges(*decode_annex_D
                                           (annex_D_disallowed_initially))

    def ident_start(self, codepoint: int) -> bool:

        return (codepoint in self.continue_ranges
                and codepoint not in self.non_start_ranges)


class UniCClang(UniC, UniClang):
    def __init__(self, lang: Preprocessor.Language):
        super().__init__(lang)
        ...
...
class UniCpp(UniBase):
    start_ranges: Ranges
    continue_ranges: Ranges

    def __init__(self, lang: Preprocessor.Language):
        super().__init__(lang)
        # Using unicode database for identifiers.

        # These strings are copied directly from the Unicode database at
        ...
        def decode_ucd(data: str) -> Iterator[Range]:
            """
            Parse codepoint ranges from ucd data and iterate on the
            ranges.
            """
            hexpat = re.compile(
                rf'(?P<first>{hexre})(\.\.(?P<last>{hexre}))?')
            for m in hexpat.finditer(data):
                first = m.group('first')
                last = m.group('last') or first
                yield Range(int(first, 16), int(last, 16) + 1)

        self.start_ranges = Ranges(*decode_ucd(ucd_start_data))
        self.continue_ranges = Ranges(*decode_ucd(ucd_continue_data))


class UniCppClang(UniCpp, UniClang):
    def __init__(self, lang: Preprocessor.Language):
        super().__init__(lang)
    ...
    def ident_start(self, codepoint: int) -> bool:
        ...
        return codepoint in self.start_ranges


class SkipException(Exception):
    """
    Raise this to indicate an invalid codepoint that should just be skipped.
    """
    pass
