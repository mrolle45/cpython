...
class CmdPreprocessor(Preprocessor):
    ...
    ignores any arguments it does not understand.''')
        ...
        argp.add_argument("-v", action="count", default=0, help="Print more log details; repeat for still more")
        ...
        try:
            ...
            print("\nINTERNAL PREPROCESSOR ERROR AT AROUND %s:%d, FATALLY EXITING NOW\n"
                % (self.currsource, self.currsource.lexer.lineno), file = sys.stderr)
            sys.exit(-99)
        finally:
            for i in self.args.inputs:
                i.close()
...
def main():
    ...
    print(f"{p.return_code} error(s)", file=sys.stderr)
